<template>
  <section class="links">
    <div class="links-wrapper">
      <div class="links-item--opinion">интересует мнение<br>профессионалов?</div>
      <div class="links-item--instagram">
        <div class="links-item--wrapper">
          <img src="~/assets/images/links_instagram.jpg" alt="Подписывайтесь на наш инстаграм!">
        </div>
      </div>
      <div class="links-item--come">заходите на наш<br>youtube канал</div>
      <div class="links-item--feedback">cмотрите отзывы практикующих педагогов</div>
      <div class="links-item--youtube">
        <div class="links-item--wrapper">
          <img src="~/assets/images/links_youtube.png" alt="Cмотрите отзывы практикующих педагогов">
        </div>
      </div>
      <div class="links-item--subscribe">
        <p>подписывайтесь на наш инстаграм!</p>
      </div>
      <div class="links-item--learn">
        <p>Узнавайте первыми о новинках, акциях и розыгрышах призов от бренда</p>
      </div>
      <div class="links-item--bgames">
        <div class="links-item--wrapper">
          <div class="links-item--redcircle">
            <img class="links-item--img" src="~/assets/images/links_bondibongames.png" alt="заходите на наш youtube канал">
          </div>
        </div>
      </div>
      <div class="links-item--wholesale">
        <p class="links-item--text">оптовые закупки</p>
        <svg class="links-item--right" width="49" height="49" viewBox="0 0 49 49" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M37.3961 25.1602C38.2546 26.0461 38.2546 27.4835 37.3961 28.3694L26.3406 39.7816C25.4821 40.6675 24.0899 40.6675 23.2314 39.7816C22.3727 38.8952 22.3727 37.4583 23.2314 36.5719L34.2867 25.1602C35.1454 24.2738 36.5374 24.2738 37.3961 25.1602Z" fill="#E2BD9E"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M11.5923 25.2579C12.4543 24.3677 13.8519 24.3677 14.714 25.2579L25.8132 36.7147C26.6752 37.6044 26.6752 39.0472 25.8132 39.9368C24.9512 40.8265 23.5535 40.8265 22.6916 39.9368L11.5923 28.4801C10.7303 27.5904 10.7303 26.1476 11.5923 25.2579Z" fill="#FFFFFF"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M24.6476 10.3119C25.8745 10.3038 26.8627 11.3107 26.8549 12.5614L26.7546 28.6619C26.7468 29.9126 25.7459 30.9325 24.5191 30.9401C23.2921 30.9483 22.3038 29.9408 22.3116 28.6907L22.412 12.5901C22.4198 11.34 23.4207 10.3195 24.6476 10.3119Z" fill="#E2BD9E"/>
          <path d="M24.5 2C36.9263 2 47 12.0734 47 24.4996C47 36.9259 36.9263 46.9998 24.5 46.9998C12.0735 46.9998 2 36.9259 2 24.4996C2 12.0733 12.0735 2 24.5 2Z" fill="black" fill-opacity="0.01" stroke="#E2BD9E" stroke-width="4"/>
        </svg>
      </div>
      <div class="links-item--arrows">
        <svg width="49" height="49" viewBox="0 0 49 49" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M37.3961 25.1602C38.2546 26.0461 38.2546 27.4835 37.3961 28.3694L26.3406 39.7816C25.4821 40.6675 24.0899 40.6675 23.2314 39.7816C22.3727 38.8952 22.3727 37.4583 23.2314 36.5719L34.2867 25.1602C35.1454 24.2738 36.5374 24.2738 37.3961 25.1602Z" fill="#E2BD9E"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M11.5923 25.2579C12.4543 24.3677 13.8519 24.3677 14.714 25.2579L25.8132 36.7147C26.6752 37.6044 26.6752 39.0472 25.8132 39.9368C24.9512 40.8265 23.5535 40.8265 22.6916 39.9368L11.5923 28.4801C10.7303 27.5904 10.7303 26.1476 11.5923 25.2579Z" fill="#FFFFFF"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M24.6476 10.3119C25.8745 10.3038 26.8627 11.3107 26.8549 12.5614L26.7546 28.6619C26.7468 29.9126 25.7459 30.9325 24.5191 30.9401C23.2921 30.9483 22.3038 29.9408 22.3116 28.6907L22.412 12.5901C22.4198 11.34 23.4207 10.3195 24.6476 10.3119Z" fill="#E2BD9E"/>
          <path d="M24.5 2C36.9263 2 47 12.0734 47 24.4996C47 36.9259 36.9263 46.9998 24.5 46.9998C12.0735 46.9998 2 36.9259 2 24.4996C2 12.0733 12.0735 2 24.5 2Z" fill="black" fill-opacity="0.01" stroke="#E2BD9E" stroke-width="4"/>
        </svg>
      </div>
      <div class="links-item--video">
        <p class="links-item--small">Веселые и увлекательные видео для детей, подростков и их родителей</p>
      </div>
    </div>
    <button class="links-down">
      <svg width="49" height="49" viewBox="0 0 49 49" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M37.3961 25.1602C38.2546 26.0461 38.2546 27.4835 37.3961 28.3694L26.3406 39.7816C25.4821 40.6675 24.0899 40.6675 23.2314 39.7816C22.3727 38.8952 22.3727 37.4583 23.2314 36.5719L34.2867 25.1602C35.1454 24.2738 36.5374 24.2738 37.3961 25.1602Z" fill="#E2BD9E"/>
        <path fill-rule="evenodd" clip-rule="evenodd" d="M11.5923 25.2579C12.4543 24.3677 13.8519 24.3677 14.714 25.2579L25.8132 36.7147C26.6752 37.6044 26.6752 39.0472 25.8132 39.9368C24.9512 40.8265 23.5535 40.8265 22.6916 39.9368L11.5923 28.4801C10.7303 27.5904 10.7303 26.1476 11.5923 25.2579Z" fill="#FFFFFF"/>
        <path fill-rule="evenodd" clip-rule="evenodd" d="M24.6476 10.3119C25.8745 10.3038 26.8627 11.3107 26.8549 12.5614L26.7546 28.6619C26.7468 29.9126 25.7459 30.9325 24.5191 30.9401C23.2921 30.9483 22.3038 29.9408 22.3116 28.6907L22.412 12.5901C22.4198 11.34 23.4207 10.3195 24.6476 10.3119Z" fill="#E2BD9E"/>
        <path d="M24.5 2C36.9263 2 47 12.0734 47 24.4996C47 36.9259 36.9263 46.9998 24.5 46.9998C12.0735 46.9998 2 36.9259 2 24.4996C2 12.0733 12.0735 2 24.5 2Z" fill="black" fill-opacity="0.01" stroke="#E2BD9E" stroke-width="4"/>
      </svg>
    </button>
  </section>
</template>

<script>
export default {
  name: "BBlinks"
}
</script>

<style lang="scss" scoped>
.links {
  background-image: url('~assets/images/background_links.png');
  background-position: center top;
  background-repeat: no-repeat;
  width: 1920px;
  height: 700px;
  z-index: -3;
  display: flex;
  flex-direction: column;
  align-items: center;
  margin: 0;
  @media (max-width: 1920px) {
    width: 100vw;
    background-position: center top;
  }
  @media (max-width: 1023px) {
    background-image: url('~assets/mobile/background_links_mobile.png');
    background-position: center top;
    background-repeat: no-repeat;
    width: 100vw;
    max-width: 480px;
    height: 923px;
  }

  &-wrapper {
    width: 1050px;
    display: grid;
    grid-template-columns: repeat(3, 350px);
    grid-template-rows: repeat(5, 120px);

    grid-template-areas:
      "opinion instagram ."
      "feedback instagram come"
      "youtube subscribe bgames"
      "youtube learn bgames"
      "wholesale arrows video";
    font-family: 'RotondaC', Helvetica, Arial, sans-serif;
    font-weight: bold;
    font-size: 20px;
    color: white;
    text-transform: uppercase;
    margin: 120px 0 0 0;
    @media (max-width: 1920px) {
      margin: 120px 0 0 0;
    }
    @media (max-width: 1024px) {
      width: 1000px;
      grid-template-columns: repeat(3, 333px);
    }
    @media (max-width: 1023px) {
      width: 100vw;
      max-width: 480px;
      margin: 100px 0 10px 0;
      display: grid;
      font-size: 17px;
      grid-template-columns: repeat(2, 240px);
      grid-template-rows: 50px repeat(2, 90px) 50px repeat(2, 90px) 50px repeat(2, 90px) 50px;

      grid-template-areas:
      "opinion opinion"
      "youtube feedback"
      "youtube feedback"
      "subscribe subscribe"
      "learn instagram"
      "learn instagram"
      "come come"
      "bgames video"
      "bgames video"
      "wholesale wholesale";
    }
    @media (max-width: 480px) {
      grid-template-columns: repeat(2, 50vw);
    }
  }

  &-arrow {
    margin: 35px 0 0 50px;
    @media (max-width: 1024px) {
      margin: 10px 0 0 50px;
    }
  }

  &-item {
    display: flex;
    justify-content: center;
    align-items: center;

    &--img {
      @media (max-width: 1023px) {
        width: 100px;
        height: 34px;
      }
    }

    &--wrapper {
      width: 173px;
      height: 173px;
      border-radius: 50%;
      background-color: #ffffff;
      display: flex;
      justify-content: center;
      align-items: center;
      box-shadow: 6.709px 4.357px 0 0 rgba(0, 0, 0, 0.09);
      @media (max-width: 1023px) {
        width: 140px;
        height: 140px;
      }
    }

    &--redcircle {
      width: 138px;
      height: 138px;
      border-radius: 50%;
      background-color: #d02027;
      display: flex;
      justify-content: center;
      align-items: center;
      @media (max-width: 1023px) {
        width: 115px;
        height: 115px;
      }
    }

    &--opinion {
      grid-area: opinion;
      font-size: 25px;
      text-align: right;
      display: flex;
      justify-content: center;
      align-items: flex-end;
      @media (max-width: 1023px) {
        align-items: center;
        text-align: center;
        max-width: 100vw;
      }
    }

    &--instagram {
      grid-area: instagram;
      display: flex;
      justify-content: center;
      align-items: flex-end;
      @media (max-width: 1023px) {
        align-items: center;
        max-width: 50vw;
      }
    }

    &--come {
      grid-area: come;
      font-size: 25px;
      display: flex;
      justify-content: center;
      align-items: flex-end;
      @media (max-width: 1023px) {
        max-width: 100vw;
      }
    }

    &--feedback {
      grid-area: feedback;
      text-align: right;
      display: flex;
      justify-content: center;
      align-items: center;
      @media (max-width: 1023px) {
        text-align: left;
        max-width: 50vw;
        font-size: 17px;
        color: #c2aad8;
      }
    }

    &--youtube {
      grid-area: youtube;
      font-size: 15px;
      position: relative;
      display: flex;
      justify-content: flex-start;
      align-items: flex-start;
      @media (max-width: 1023px) {
        justify-content: center;
        align-items: center;
        max-width: 50vw;
      }

      &:after {
        content: '';
        background-image: url('assets/images/arrow.png');
        width: 130px;
        height: 58px;
        position: absolute;
        transform: rotate(160deg);
        top: -20px;
        right: 100px;
        @media (max-width: 1023px) {
          display: none;
          visibility: hidden;
        }
      }
    }

    &--subscribe {
      grid-area: subscribe;
      font-size: 25px;
      display: flex;
      justify-content: center;
      align-items: flex-end;
      position: relative;
      @media (max-width: 1023px) {
        text-align: center;
        align-items: center;
        max-width: 100vw;
      }

      &:after {
        content: '';
        background-image: url('assets/images/arrow.png');
        width: 130px;
        height: 58px;
        position: absolute;
        transform: rotate(-30deg);
        top: -45px;
        left: 10px;
        @media (max-width: 1023px) {
          display: none;
          visibility: hidden;
        }
      }
    }

    &--learn {
      grid-area: learn;
      display: flex;
      justify-content: center;
      align-items: flex-start;
      @media (max-width: 1023px) {
        align-items: center;
        text-align: right;
        max-width: 50vw;
        justify-content: center;
        font-size: 17px;
        color: #c2aad8;
      }
    }

    &--bgames {
      grid-area: bgames;
      display: flex;
      justify-content: center;
      align-items: center;
      @media (max-width: 1023px) {
        max-width: 50vw;
      }
    }

    &--arrows {
      grid-area: arrows;
      display: flex;
      justify-content: space-around;
      align-items: center;
      @media (max-width: 1023px) {
        grid-area: unset;
        display: none;
        visibility: hidden;
      }
    }

    &--wholesale {
      grid-area: wholesale;
      display: flex;
      justify-content: center;
      align-items: center;
    }

    &--video {
      grid-area: video;
      display: flex;
      justify-content: center;
      align-items: flex-start;
      position: relative;
      @media (max-width: 1023px) {
        max-width: 170px;
        align-items: center;
      }

      &:after {
        content: '';
        background-image: url('assets/images/arrow.png');
        width: 130px;
        height: 58px;
        position: absolute;
        transform: rotate(-30deg);
        top: -70px;
        left: 15px;
        @media (max-width: 1023px) {
          display: none;
          visibility: hidden;
        }
      }
    }

    &--small {
      font-size: 20px;
      text-align: left;
      margin: 10px 0 0 0;
      @media (max-width: 1023px) {
        font-size: 17px;
        color: #c2aad8;
      }
    }

    &--text {
      font-size: 20px;
      font-family: Manrope-Regular, Arial, sans-serif;
      color: #ffffff;
      text-transform: uppercase;
      line-height: 26px;
      display: flex;
      justify-content: center;
      align-items: center;
      width: 300px;
      height: 49px;
      background-color: #d2beae;
      border-radius: 24px;
    }

    &--right {
      transform: rotate(270deg);
      margin: 0 0 0 15px;
      @media (max-width: 1023px) {
        display: none;
        visibility: hidden;
      }
    }
  }

  &-button {
    background: transparent;
    border: none;
    cursor: pointer;
    z-index: 2;
    padding: 0;
    width: 49px;
    height: 49px;
    @media (max-width: 1023px) {
      display: none;
      visibility: hidden;
    }

    &--right {
      transform: rotate(270deg);
    }

    &--wrapper {
      display: flex;
      flex-direction: row;
      width: 170px;
      justify-content: space-between;
      margin: 40px 0 0 15px;
    }
  }

  &-down {
    background: transparent;
    border: none;
    cursor: pointer;
    z-index: 2;
    padding: 0;
    width: 49px;
    height: 49px;
    display: none;
    visibility: hidden;
    @media (max-width: 1023px) {
      display: block;
      visibility: visible;
    }
  }
}
</style>